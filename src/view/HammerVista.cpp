...
void
HammerVista::mostrar(int x, int y, EstadoHammer estado, SDL_RendererFlip flip)
{
  const SDL_Rect srcRect{
    x_pos + (static_cast<int>(estado) * src_width), y_pos, src_width, src_height
  };
  const SDL_Rect dstRect = { x
                               + static_cast<int>(estado) * ANCHO_MARIO
                                   * (flip == SDL_FLIP_NONE ? -1 : 1),
                             y + y_offset
                               - ALTO_MARIO * (1 - static_cast<int>(estado)),
                             dst_width,
                             dst_height };
