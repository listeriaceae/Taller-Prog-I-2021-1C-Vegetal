...
static constexpr SDL_Rect usernameRect = {
  (int)(TEXT_BUTTON_X * ANCHO_PANTALLA / (float)ANCHO_NIVEL + 0.5f),
...
static constexpr SDL_Rect doneRect = {
  (int)(DONE_BUTTON_X * ANCHO_PANTALLA / (float)ANCHO_NIVEL + 0.5f),
...
{
  return usernameRect.y <= y && y <= usernameRect.y + usernameRect.h && usernameRect.x <= x && x <= usernameRect.x + usernameRect.w;
}

constexpr bool
  mouseOnPasswordButton(int x, int y)
{
  return passwordRect.y <= y && y <= passwordRect.y + passwordRect.h && passwordRect.x <= x && x <= passwordRect.x + passwordRect.w;
}

constexpr bool
  mouseOnDoneButton(int x, int y)
{
  return doneRect.y <= y && y <= doneRect.y + doneRect.h && doneRect.x <= x && x <= doneRect.x + doneRect.w;
}
}// namespace

StartPage::StartPage() : resultMsg{ EMPTY_STR } {}
...
{
  const punto<int> pos{ static_cast<int>((TEXT_BUTTON_X + 2 * RESIZE) * ANCHO_PANTALLA / (float)ANCHO_NIVEL),
    static_cast<int>(ERROR_MSG_Y * ALTO_PANTALLA / (float)ALTO_NIVEL) };
